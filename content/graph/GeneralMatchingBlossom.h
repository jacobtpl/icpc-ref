...
struct MaxMatching {
	...
	void init(int _N) { N = _N; adj = vector<vi>(N+1); 
	...
	void match(int p, int b) {
		...
		if (!label[p].second) mate[b] = label[p].first, match(label[p].first,b); // vertex label
		else match(label[p].first,label[p].second), match(label[p].second,label[p].first); // edge label
	}
