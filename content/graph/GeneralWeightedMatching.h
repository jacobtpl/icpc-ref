...
template<int SZ> struct WeightedMatch { 
	...
	void qPush(int x) {
		if (x <= N) q.push(x);
		else for(auto &t:flo[x]) qPush(t); }
	void setSt(int x, int b) {
		st[x] = b; if (x > N) for(auto &t:flo[x]) setSt(t,b); }
	...
	void addBlossom(int u, int anc, int v) { 
		...
		for(auto &xs:flo[b]) { // find tightest constraints
	...
	void expandBlossom(int b) {
		for(auto &t:flo[b]) setSt(t,t); // undo setSt(b,b)
		...
		S[xr] = 1, par[xr] = par[b];
		rep(i,pr+1,sz(flo[b])) { // matches don't change
	...
	bool matching() {
		...
		while (1) {
			...
			rep(b,N+1,NX+1) if (st[b] == b && S[b] != -1) 
				lab[b] += (S[b] == 0 ? 1 : -1)*d*2;
			q = queue<int>();
			rep(x,1,NX+1) if (st[x]==x && slack[x] // new tight edge
	...
	pair<ll,int> calc() {
		...
		rep(u,1,N+1) lab[u] = wMax; // start high and decrease
		int num = 0; ll wei = 0; while (matching()) ++num;
		rep(u,1,N+1) if (match[u] && match[u] < u) 
